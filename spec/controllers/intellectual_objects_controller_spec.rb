...
describe IntellectualObjectsController do
  ...
  describe 'create an object' do
    ...
    def sample_object
      {
          institution_id: user.institution_pid,
          title: 'Test Title',
          access: 'consortia',
          description: '',
    ...
    describe 'when signed in' do
      ...
      it 'should use the institution parameter in the URL, not from the json' do
        expect {
          post :create, institution_identifier: user.institution_identifier, intellectual_object: {title: 'Foo', institution_id: user.institution_pid, identifier: 'test.edu/123', access: 'restricted'}, format: 'json'
          expect(response.code).to eq '201'
          expect(assigns(:intellectual_object).title).to eq 'Foo'
